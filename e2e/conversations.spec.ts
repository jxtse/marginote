...
test(`${provider}: connects a delivery, discusses, proposes edits and continues after acceptance`, async ({ page }) => {
  ...
  try {
    ...
    server = await MarginoteServer.start({ root, port: 0, git: false, webRoot: resolve("packages/web/dist"), conversationProvider: {
      fork: async origin => { expect(origin).toEqual({ provider, sessionId: "parent", turnId: "delivery" }); forks++; return "child"; },
    ...
    await expect(page.locator("#comments .reply")).toHaveCount(0);
    ...
    await expect(page.locator("#comments .reply")).toHaveCount(5);
    ...
    await page.getByRole("button", { name: "Send reply" }).click();
    await expect(page.locator("#comments .reply")).toHaveCount(7);
