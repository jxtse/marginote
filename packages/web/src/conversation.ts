...
export interface ConversationStatus {
  ...
  state: "idle" | "working" | "approval" | "error";
  error: string | null;
...
export class ConversationPanel {
  ...
  render(doc: string | null, status: ConversationStatus | null): void {
    ...
    title.textContent = `${providerName(status.origin.provider)} conversation · ${{ idle: "Ready", working: "Working…", approval: "Your approval needed", error: "Needs attention" }[status.state]}`;
    title.setAttribute("role", "status");
    const note = document.createElement("p");
    note.textContent = "New comments continue the conversation that produced this document.";
    ...
    if (["idle", "error"].includes(status.state)) action("Disconnect conversation", "disconnect", {});
  }
}
